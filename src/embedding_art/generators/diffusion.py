...
class SDXLDiffusionGenerator:
    ...
    def generate(
        ...
        **kwargs
        ...
        logger.info(f"Running ImageBind Guided SDXL Generation (scale={imagebind_guidance_scale})...")
        ...
        try:
            ...
            latents = self.pipeline.prepare_latents(
                ...
                prompt_embeds.dtype, self.device, None
            )
                ...
                for i, t in enumerate(timesteps):
                    ...
                    with torch.no_grad():
                        ...
                        if guidance_scale > 1.0:
                             added_cond_kwargs["text_embeds"] = torch.cat([negative_pooled_prompt_embeds, add_text_embeds])
                    ...
                    if should_guide:
                         for k in range(GRADIENT_STEPS):
                             ...
                             latents_in = latents.to(self.pipeline.vae.dtype) / self.pipeline.vae.config.scaling_factor
                             ...
                             # Cosine Distance
                             sim = F.cosine_similarity(current_embedding, target_embedding.to(current_embedding.device), dim=-1).mean()
                             loss = 1.0 - sim
    ...
    def init_latent(self, seed: int | None = None, scale: float = DEFAULT_SEED_SCALE) -> torch.Tensor:
        """
